...
> Throughout the K3s documentation, you will see some options that can be passed in as both command flags and environment variables. For help with passing in options, refer to [How to Use Flags and Environment Variables.](/installation/install-options/how-to-flags/how-to-flags)

### Options for Installation with Script

As mentioned in the [Quick-Start Guide](/quick-start/quick-start), you can use the installation script available at https://get.k3s.io to install K3s as a service on systemd and openrc based systems.
...
For details on configuring the K3s server, refer to the [server configuration reference.](/installation/install-options/server-config/server-config)


### Registration Options for the K3s Agent

For details on configuring the K3s agent, refer to the [agent configuration reference.](/installation/install-options/agent-config/agent-config)
