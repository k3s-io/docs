...
4. [プライベートレジストリの設定](images/private-registry.md)ガイドに従って、`registries.yaml`ファイルを作成および設定します。
...
埋め込み分散レジストリミラーの有効化に関する詳細は、[埋め込みレジストリミラー](./images/registry-mirror.md)ドキュメントを参照してください。
