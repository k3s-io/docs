...
访问 [Project Calico 文档](https://docs.tigera.io/calico/)网站。按照步骤安装 Canal。修改 Canal YAML 以便在 `container_settings` 中允许 IP 转发，例如：
...
:::caution 警告
嵌入式 etcd 不会使用外部 IP 进行通信。如果使用嵌入式 etcd，所有 Server 节点必须可以通过其私有 IP 相互访问。
:::
