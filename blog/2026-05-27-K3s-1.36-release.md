...
Kubernetes 1.36 has officially landed! Following our continuous commitment to providing a lightweight yet production-grade Kubernetes distribution, the K3s team and community have put together another incredible milestone release. 🥳

<!-- truncate -->

This cycle, we focused on strengthening our core infrastructure—bringing major storage innovations, highly requested security enhancements, and empowering advanced workloads like AI and machine learning via modern container runtime features. Here is what's new in the world of K3s.


## Key Features and Improvements ✨

### Core Storage Evolution: etcd 3.6 & SQLite Upgrades 💾
We have officially upgraded our bundled etcd to the v3.6 series. While this brings massive performance improvements and optimizations upstream, it introduces a critically important caveat for administrators running High Availability (HA) clusters: etcd v3.6 changes the data directory structure, which prevents a clean downgrade to v3.5 once your cluster has updated. If you ever need to rollback a v1.36 cluster to v1.35, you must follow specific manual snapshot restoration and node cleaning procedures. Be sure to review our dedicated migration guide before upgrading production environments.
...
Customizing your container runtime configuration just got much cleaner. K3s now supports default imports inside our containerd base configuration templates. This architectural change allows administrators to modularly inject runtime configurations and snippets without having to rewrite or completely overwrite the entire default config.toml.tmpl file.

### Dynamic Security: Secrets Encryption for Existing Clusters 🔐
Security is never static, and neither should your cluster configuration be. Historically, enabling at-rest Secrets Encryption required setting it up during the initial cluster bootstrapping phase. With v1.36, you can now seamlessly enable secrets encryption on existing, running clusters without needing to recreate them, making it vastly easier to harden legacy deployments.

### AI & Hardware Acceleration: Containerd 2.2 with NRI Support 🚀
We have bumped our embedded container runtime to Containerd 2.2, which introduces out-of-the-box support for the Node Resource Interface (NRI). This is a game-changer for clusters running edge AI and heavy GPU workloads, enabling cleaner integration with the NVIDIA Container Toolkit and highly dynamic, runtime-level device management.
